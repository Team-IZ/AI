...
def test_fetch_failure_code_translation_values_are_all_db_legal():
    """fetch.py 어휘 전체가 DB 15종 밖으로 새지 않는지. DB에 이름이 없는 코드
    (JOB_ONLY 2종, 그리고 나중에 늘어날 새 코드)는 SOURCE_UNREACHABLE로 떨어진다."""
    from typing import get_args
    ...
    vocabulary = fetch_engine.VERIFICATION_FAILURE_CODES | fetch_engine.JOB_ONLY_FAILURE_CODES
