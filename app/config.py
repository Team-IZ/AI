...
class Settings(BaseSettings):
    ...
    #   WHY: 20개 후보 × 3역할(세션채점/보고서/면담브리프) × 3반복 실측(Team-IZ-AI-
    #        interview-brief worktree, benchmarks/deepseek_v4_flash_replacement.py) +
    #        Claude Sonnet 동일 파이프라인 응답을 품질 기준점으로 삼은 재채점 결과,
    #        minimax-m3가 채점(90%)·보고서(93%) 조합에서 종합 1위(76.7%)였다.
    #   COST: minimax-m3는 D116 4축 벤치마크에서 소요시간이 느린 편(채점 31.4s,
    #         옛 deepseek-v4-flash 7~12초 대비 느려짐) — 속도보다 품질을 우선한 결정.
    #         면담 브리프 역할에서는 3회 중 1회 타임아웃(구조검사 통과율 67%)이라
    #         그 역할엔 이 값을 안 쓴다(model_code_interview_brief는 별도로
    #         openai/gpt-oss-120b로 간다, feat/interview-brief 브랜치 참고).
    #   EXIT: 다음 deprecate 통보가 오면 같은 하니스(20개 후보 셔틀리스트는
    #         이 결정 당시 살아있던 모델 기준이라 갱신 필요)로 재실행.
