...
# 바뀌었다(config.py 참고). 위 6.9~11.7초 분포는 deepseek-v4-flash 기준이라 이제
# 근거가 낡았다 -- 20초를 그대로 둔 건 데이터가 있어서가 아니라 **아직 minimax-m3의
# 단일 호출(재시도 제외) 지연분포를 재실측 안 해서**다(벤치마크의 mean_elapsed_s는
# 재시도 포함 합산값이라 이 상수의 근거로 못 쓴다 -- CLAUDE.md §13 Data-First
# Numerics). 재실측 전까지 20초 유지가 안전한 이유: minimax-m3는 D116에서 관측된
# 실패 모드도 deepseek-v4-flash와 같은 "정체 아닌 조기 실패" 계열이라(529류) 상한을
# 낮출 유인이 없고, 올려야 할 이유(성공 호출이 20초를 넘겨 죽는지)는 아직 미확인이다.
