...
#
# 실측(2026-07-31, mistral-medium-3.5)에서 지연이 **이봉 분포**로 나왔다.
#
#   성공  3.2 · 4.0 · 4.4 · 4.5 · 5.2초        ← 전부 6초 미만
#   정체  306 · 604 · 910초                     ← 중간값이 없다
#
# 프롬프트 크기 탓이 아니다. 같은 스테이지·같은 코드가 어떤 때는 3초, 어떤 때는
# 수 분이다 — 공급자 쪽 큐에 걸리는 것으로 보인다.
#
# 그래서 상한을 정상 구간 바로 위에 둔다. 정상 호출을 죽이지 않으면서 정체를
# 즉시 걸러낸다. 시도마다 다른 키로 나가므로 재시도가 독립 시행이 된다.
#
# 2026-08-01 스트리밍 실측(draw 20회)으로 20초 → 8초로 내렸다.
#
#   정상 draw   첫 토큰 TTFT 중앙값 0.62초, 8개 중 7개가 2.5초 이내
#   정체 draw   **30초를 기다려도 첫 토큰이 0개다.** 느린 게 아니라 영영 안 온다
#
# 즉 정체 draw를 오래 기다리는 것은 100% 낭비다. 8초면 정상 draw를 안 죽이면서
# (관측된 최대 완료 11.2초 중 TTFT는 9.28초가 최대) 정체를 빨리 버린다.
SESSION_TIMEOUT_S = 8.0

# 세션 경로 재시도 횟수.
#
# ⚠️ **타임아웃과 함께 줄이면 안 된다.** 20초×4회(예산 80초)에서 8초×5회(예산 40초)로
# 바꿨더니 평균은 21.3초 → 13.1초로 좋아졌는데 하드 실패가 0건 → 9건(25회 중)으로 늘었다.
# 실패는 전부 "5회 연속 정체". 정체율 79%에서 0.79^5 = 31%이니 예상된 값이었다.
#
# 그래서 타임아웃만 줄이고 시행 횟수로 예산을 되돌린다. 정체가 독립 시행이라
# (시도마다 다른 키) 시행을 늘리는 쪽이 기다리는 쪽보다 언제나 이득이다.
#
#   20초 × 4회 = 예산 80초, 기회 4번   정체율 79%면 실패 39%
#    8초 × 5회 = 예산 40초, 기회 5번   실패 31% — 예산을 반토막 낸 대가
#    8초 × 10회 = 예산 80초, 기회 10번  실패  9%  ← 지금. 둘 다 이긴다
