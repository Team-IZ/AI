...
#
# 그래서 상한을 정상 구간 바로 위에 둔다. 20초면 정상 호출을 죽이지 않으면서
# 정체를 즉시 걸러낸다. 시도마다 다른 키로 나가므로 재시도가 독립 시행이 된다.
#
#   60초 × 2회 = 최악 120초, 성공 기회 2번   ← 실측에서 3턴이 이렇게 죽었다
#   20초 × 4회 = 최악  80초, 성공 기회 4번
SESSION_TIMEOUT_S = 20.0

# 세션 경로 재시도 횟수. 정체가 독립적으로 발생한다는 전제라 시행을 늘리는 쪽이 이득이다.
