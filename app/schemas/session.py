...
class Question(BaseSchema):
    """ 지금 물어보는 질문 하나 """
    dp_id: str                    # decision_point 키
    depth_level: str              # 문답 깊이 L1/L2/...
    sequence_no: int
    question_text: str
    code_context: CodeContext | None = None
    
class Progress(BaseSchema):
    dp_index: int                 # 몇 번째 DP인지(1부터)
    dp_total: int
...
class SessionStart(BaseSchema):
    ...
    selected_finding_ids: list[str] = Field(default_factory=list, description="세션에 포함할 DP. 생략 시 전체")
