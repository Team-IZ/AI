...
| 테스트 | 58 passed |
| 다음 | **🔴 T10 기능 완성 → T9 배포.** 2026-08-02 동결: 6개 기능을 끝까지 완성해 배포하고 속도 개선은 그다음. **작업 기준은 §T10** — 앞선 절과 충돌하면 T10이 이긴다. 착수는 **p04-1**(2·3·4가 전부 이 출력을 기다린다) |
...
| 파일:줄 | 지금 | 되어야 할 것 |
|---|---|---|
| `schemas/analysis.py:109` | `FROZEN_AXES = ("L1", "L2")` | **4축 전부.** `_check_axis_rules`의 `else` 분기(L3·L4는 `questionText` null + 힌트 빈 배열)가 죽는다 |
| `engines/analysis/scoring.py:92` | `FROZEN_AXES`/`ADAPTIVE_AXES` 분리 | 적응형 축 없음 |
| `engines/analysis/questions.py` | `_normalize`가 L3·L4를 버린다 | 4개를 다 쓴다 |
| `schemas/report.py` | 세션 단위 | 문제 단위(`problemId`) |
| `engines/__init__.py:20` | `real` 모드가 `NotImplementedError` | 조립기 배선 |
