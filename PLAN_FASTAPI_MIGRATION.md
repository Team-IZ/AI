...
```
contextType    ANALYSIS_JOB · ASSESSMENT_SESSION · REPORT_SNAPSHOT ·
               CURRICULUM_ANALYSIS · INTERVIEW_BRIEF               ← 값 집합 확정(2026-08-07)
failureCode    TIMEOUT · RATE_LIMITED · PROVIDER_ERROR · INVALID_JSON · CONTEXT_OVERFLOW
               (DB CHECK는 8종. 나머지 3종은 다중 인스턴스 폴백 체계가 없어 도달 불가)
idempotencyKey {요청키|contextId}:{contextType}:{순번}
```

🔴 **`SUBMISSION`은 폐기다** (2026-08-07 백엔드 회신, 제안서 D). `/analyses`는 `ANALYSIS_JOB` + `contextId=jobId`를 쓴다 — 같은 제출을 재분석하면 두 실행의 토큰이 한 덩어리로 합쳐져 어느 쪽이 비쌌는지 안 보였다.

🔴 **`contextId`는 NULL 허용 컬럼이다**(DB에서 이 컬럼만). 면담 브리프는 AI가 `brief_id`를 받은 적이 없어 **null을 보낸다** — `REPORT_SNAPSHOT`·`CURRICULUM_ANALYSIS`가 쓰는 "가짜 jobId를 넣고 Spring이 교체" 우회조차 필요 없다. 대신 `request_id`·`trace_id`·`idempotency_key`가 NOT NULL이라 폴백 사슬이 `contextId → idempotency-key 헤더 → x-trace-id`로 내려간다(`app/usage.py`). 그래서 **면담 브리프만 멱등키 헤더가 필수다** — 없으면 `":INTERVIEW_BRIEF:1"`이 되어 두 번째 호출이 전역 UNIQUE에 걸린다.

🔴 **`idempotencyKey` 형식이 2026-08-03에 바뀌었다.** 예전엔 한 작업의 모든 행에 요청 헤더 키를 그대로 박아 **행마다 키가 같았다.** 그대로 두면 Spring이 여러 콜을 한 행으로 합쳐 토큰이 사라진다.

**`featureCode`** — DB CHECK 6종 전부를 AI가 쓴다.

| AI 단계 | 값 |
|---|---|
| 코드 분석 문서 (p04-1) · 요구사항 P/F (p04-2) | `CODE_ANALYSIS` |
| 문제 선정 (p04-3) · 질문·힌트 동결 (p04-4·p04-7) | `CODE_SESSION` |
| 답변 채점 (p04-5) | `ANSWER_EVALUATION` |
| 보고서 (p04-6) | `REPORT_GENERATION` |
| 교안 분석 (p01) | `CURRICULUM_ANALYSIS` |
| 면담 브리프 (ib-1) | `INTERVIEW_BRIEF_GENERATION` |
...
| 날짜 | 내용 |
|---|---|
| 2026-08-15 | **`interviewSourceId` 앵커 폴백 + `questionType` 노출**(`feat/interview-brief-source-anchor`) — 백엔드 합동 테스트에서 **200 OK인데 브리프 항목이 0건 저장**되는 것이 잡혔다. 원인은 실제 DDL(`PostgreSQL_v07_Table_DDL_2026_08_07_09_06.sql`)에서 `interview_brief_item.interview_source_id`가 **`UUID NOT NULL` + FK**라는 것 — 백엔드 INSERT의 `WHERE s.interview_source_id = ?`가 0행이라 **예외도 없이 조용히 누락된다.** 🔴 **기준 문서를 DDL 실물로 바꿨다** — 8/12에 드롭을 철회한 근거였던 `테이블정의서_2026-08-06`의 `source_type='MANUAL' AND interview_source_id IS NULL` CHECK가 **08-07 재설계에서 사라졌다**(그 테이블은 컬럼 구성부터 다르다: `title`·`summary`·난이도4종 → `question_text`·`question_rationale`). 공통 78테이블 중 **17개가 어긋나 있었고** 신규 2개(`assessment_problem_reference` **부활** · `report_generation_item`)가 있다. 조치는 **드롭도 null도 아닌 앵커**다 — `RAPPORT`+관찰메모 1건이면 그 메모, 그 밖은 `attemptInterviewSourceId`(메모 2건 이상이면 어느 것을 썼는지 몰라 attempt로 떨어뜨린다). **모델에게 id를 강제하지 않는 원칙은 그대로** — 모델이 낸 공백을 코드가 결정적으로 메운다. 응답의 `interviewSourceId`는 `str \| None` → **필수 `str`**, `questionType`을 노출한다(백엔드가 MANUAL 자리 생길 때 판정 근거로 보관). 스텁도 같은 앵커를 쓴다. 백엔드는 **수정 0건**(`validateSourceIds`·INSERT 그대로 통과)이고 DB 변경도 0건이다. 백엔드가 attempt 부재를 `NO_ASSESSMENT_ATTEMPT`(409)로 막아 앵커가 비는 경로를 없앴다. ⚠️ **임시 다리다** — `interviewSourceId`만 보면 라포 질문이 "시도 근거"처럼 보인다. DB에 MANUAL 자리가 생기면 폴백을 끄면 된다. 실호출 확인(9.3초·재시도 0·null 0건). **435 tests** |
| 2026-08-12 | **팀원 PR#28 통합 + 근거 없는 항목 드롭 철회**(`fix/interview-brief-composition`) — 팀원 브랜치가 `da5d51b`에서 5일 묵은 채 재개돼 D-ib5가 develop `e2121ae`와 **중복**이었다. 그 커밋만 드롭하고 새 작업 3개(캐주얼 페르소나·5카테고리 고정 구성·temperature 0.3)를 cherry-pick했다. 🔴 **`observationNotes`가 비면 드롭(§3 D-1②, PR#18)은 되돌렸다** — 근거였던 "`interview_brief_item.interview_source_id`가 UUID NOT NULL"이 테이블정의서(2026-08-06)와 어긋난다. 그 컬럼은 nullable이고 CHECK가 `source_type='MANUAL' AND interview_source_id IS NULL`을 명시적으로 허용한다 — 라포·일반 질문은 설계상 근거가 없어 그 자리가 필요하다(백엔드 통보 필요). 함께 **QNA 근거의 `axis_code=='L2'` 필터 삭제**(`NOT_PASSED`는 "이 축에서 문제가 끝났다"라 문제당 최대 1개다 — L2로 좁히면 L1에서 끝난 학생, 곧 면담 1순위의 문답 질문이 0개가 된다) · **스텁 id를 카테고리별로 정확히 배정** (위험 질문엔 위험 사유 id, 문답 질문엔 그 단계 id. 허용 집합에서 위치로 뽑던 걸 교체). **431 tests** |
