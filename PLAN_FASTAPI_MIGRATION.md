...
터널 URL이 cloudflared 재시작마다 바뀐다. 무료 도메인 + Caddy. **백엔드는 그 전까지 AI 주소를 설정값으로 갖고 있어야 한다.**
...
**지금 하지 않는다.** 선별 방식은 문제의 *질*이지 파이프라인 *동작*이 아니다.
...
⚠️ **App Runner는 2026-04-30부터 신규 고객 차단이라 쓸 수 없다.** 다시 검토하지 말 것.
